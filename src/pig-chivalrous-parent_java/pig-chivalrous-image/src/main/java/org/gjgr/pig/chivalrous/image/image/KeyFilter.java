...
/**
 * An experimental filter which can be used for keying against a clean shot. Given a source image, a clean image and a
 * destination image, the filter replaces all pixels in the source which nearly equal the equivalent clean pixel by
 * destination pixels.
 */
        ...
        if (destination != null && cleanImage != null) {
            ...
            for (int y = 0; y < height; y++) {
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    // return Math.abs(r1-r2) <= tolerance && Math.abs(g1-g2) <= tolerance && Math.abs(b1-b2) <=
                    // tolerance;
