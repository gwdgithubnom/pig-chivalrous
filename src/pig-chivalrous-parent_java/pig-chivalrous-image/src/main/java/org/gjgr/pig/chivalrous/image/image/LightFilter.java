    ...
    protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
        ...
        if (bumpSource == BUMPS_FROM_IMAGE || bumpSource == BUMPS_FROM_IMAGE_ALPHA || bumpSource == BUMPS_FROM_MAP
            ...
            if (bumpSoftness != 0) {
                ...
                /*
                 * for (int i = 0; i < 3; i++ ) { BoxBlurFilter.blur( bumpPixels, tmpPixels, bumpWidth, bumpHeight,
                 * (int)bumpSoftness ); BoxBlurFilter.blur( tmpPixels, softPixels, bumpHeight, bumpWidth,
                 * (int)bumpSoftness ); }
                 */
                ...
                GaussianFilter.convolveAndTranspose(kernel, bumpPixels, tmpPixels, bumpWidth, bumpHeight, true, false,
                        false, GaussianFilter.WRAP_EDGES);
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                if (normal.z >= 0) {
                    ...
                    // Shade the pixel
                    Color4f c =
                            phongShade(position, viewpoint, normal, diffuseColor, specularColor, material, lightsArray);
