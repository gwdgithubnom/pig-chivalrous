...
/**
 * A class containing static convenience methods for locating <code>ImageReader</code>s and <code>ImageWriter</code>s,
...
public final class ImageIO {
    ...
    /**
     * Scans for plug-ins on the application class path, loads their service provider classes, and registers a service
     * provider instance for each one found with the <code>IIORegistry</code>.
     *
     * <p>
     * This method is needed because the application class path can theoretically change, or additional plug-ins may
     * become available. Rather than re-scanning the classpath on every invocation of the API, the class path is scanned
     * automatically only on the first invocation. Clients can call this method to prompt a re-scan. Thus this method
     * need only be invoked by sophisticated applications which dynamically make new plug-ins available at runtime.
     *
     * <p>
     * The <code>getResources</code> method of the context <code>ClassLoader</code> is used locate JAR files containing
     * files named <code>META-INF/services/javax.imageio.spi.</code><i>classname</i>, where <i>classname</i> is one of
     * <code>ImageReaderSpi</code>, <code>ImageWriterSpi</code>, <code>ImageTranscoderSpi</code>,
     * <code>ImageInputStreamSpi</code>, or <code>ImageOutputStreamSpi</code>, along the application class path.
     *
     * <p>
     * The contents of the located files indicate the names of actual implementation classes which implement the
     * aforementioned service provider interfaces; the default class loader is then used to load each of these classes
     * and to instantiate an instance of each class, which is then placed into the registry for later retrieval.
     *
     * <p>
    ...
    private static synchronized CacheInfo getCacheInfo() {
        ...
        CacheInfo info =
                (CacheInfo) context.get(CacheInfo.class);
        if (info == null) {
            info = new CacheInfo();
    ...
    /**
     * Returns the current value set by <code>setUseCache</code>, or <code>true</code> if no explicit setting has been
     * made.
     *
     ...
     *
     * <p>
     * When reading from a standard <code>InputStream</code>, it may be necessary to save previously read information in
     * a cache since the underlying stream does not allow data to be re-read. Similarly, when writing to a standard
     * <code>OutputStream</code>, a cache may be used to allow a previously written value to be changed before flushing
     * it to the final destination.
     *
     * <p>
     * The cache may reside in main memory or on disk. Setting this flag to <code>false</code> disallows the use of disk
     * for future streams, which may be advantageous when working with small images, as the overhead of creating and
     * destroying files is removed.
     *
     * <p>
     * On startup, the value is set to <code>true</code>.
     *
     * @param useCache a <code>boolean</code> indicating whether a cache file should be used, in cases where it is
     *            optional.
     *
     * @see #getUseCache
     */
    ...
    /**
     * Returns the current value set by <code>setCacheDirectory</code>, or <code>null</code> if no explicit setting has
     * been made.
     *
     * @return a <code>File</code> indicating the directory where cache files will be created, or <code>null</code> to
     ...
     * system-dependent default temporary-file directory is to be used. If <code>getUseCache</code> returns false, this
     * value is ignored.
     *
     ...
     *
     * @exception SecurityException if the security manager denies access to the directory.
     * @exception IllegalArgumentException if <code>cacheDir</code> is non-<code>null</code> but is not a directory.
     *
    ...
    /**
     * Returns an <code>ImageInputStream</code> that will take its input from the given <code>Object</code>. The set of
     * <code>ImageInputStreamSpi</code>s registered with the <code>IIORegistry</code> class is queried and the first one
     * that is able to take input from the supplied object is used to create the returned <code>ImageInputStream</code>.
     * If no suitable <code>ImageInputStreamSpi</code> exists, <code>null</code> is returned.
     *
     * <p>
     * The current cache settings from <code>getUseCache</code>and <code>getCacheDirectory</code> will be used to
     * control caching.
     *
    ...
    /**
     * Returns an <code>ImageOutputStream</code> that will send its output to the given <code>Object</code>. The set of
     * <code>ImageOutputStreamSpi</code>s registered with the <code>IIORegistry</code> class is queried and the first
     * one that is able to send output from the supplied object is used to create the returned
     * <code>ImageOutputStream</code>. If no suitable <code>ImageOutputStreamSpi</code> exists, <code>null</code> is
     * returned.
     *
     * <p>
     * The current cache settings from <code>getUseCache</code>and <code>getCacheDirectory</code> will be used to
     * control caching.
     *
     * @param output an <code>Object</code> to be used as an output destination, such as a <code>File</code>, writable
     *            <code>RandomAccessFile</code>, or <code>OutputStream</code>.
     *
     * @return an <code>ImageOutputStream</code>, or <code>null</code>.
     *
     * @exception IllegalArgumentException if <code>output</code> is <code>null</code>.
     * @exception IOException if a cache file is needed but cannot be created.
     *
    ...
    /**
     * Returns an array of <code>String</code>s listing all of the informal format names understood by the current set
    ...
    /**
     * Returns an array of <code>String</code>s listing all of the file suffixes associated with the formats understood
    ...
    /**
     * Returns an <code>Iterator</code> containing all currently registered <code>ImageReader</code>s that claim to be
     * able to decode the supplied <code>Object</code>, typically an <code>ImageInputStream</code>.
     *
     * <p>
    ...
    /**
     * Returns an <code>Iterator</code> containing all currently registered <code>ImageReader</code>s that claim to be
     * able to decode the named format.
     *
     * @param formatName a <code>String</code> containing the informal name of a format (<i>e.g.</i>, "jpeg" or "tiff".
     *
     * @return an <code>Iterator</code> containing <code>ImageReader</code>s.
     *
    ...
    /**
     * Returns an <code>Iterator</code> containing all currently registered <code>ImageReader</code>s that claim to be
     * able to decode files with the given suffix.
     *
     * @param fileSuffix a <code>String</code> containing a file suffix (<i>e.g.</i>, "jpg" or "tiff").
     *
     * @return an <code>Iterator</code> containing <code>ImageReader</code>s.
     *
    ...
    /**
     * Returns an <code>Iterator</code> containing all currently registered <code>ImageReader</code>s that claim to be
     * able to decode files with the given MIME type.
     *
     * @param MIMEType a <code>String</code> containing a file suffix (<i>e.g.</i>, "image/jpeg" or "image/x-bmp").
     *
     * @return an <code>Iterator</code> containing <code>ImageReader</code>s.
     *
    ...
    /**
     * Returns an <code>Iterator</code> containing all currently registered <code>ImageWriter</code>s that claim to be
     * able to encode the named format.
     *
     * @param formatName a <code>String</code> containing the informal name of a format (<i>e.g.</i>, "jpeg" or "tiff".
     *
    ...
    /**
     * Returns an <code>Iterator</code> containing all currently registered <code>ImageWriter</code>s that claim to be
     * able to encode files with the given suffix.
     *
     * @param fileSuffix a <code>String</code> containing a file suffix (<i>e.g.</i>, "jpg" or "tiff").
     *
     * @return an <code>Iterator</code> containing <code>ImageWriter</code>s.
    ...
    /**
     * Returns an <code>Iterator</code> containing all currently registered <code>ImageWriter</code>s that claim to be
     * able to encode files with the given MIME type.
     *
     * @param MIMEType a <code>String</code> containing a file suffix (<i>e.g.</i>, "image/jpeg" or "image/x-bmp").
     *
     * @return an <code>Iterator</code> containing <code>ImageWriter</code>s.
    ...
    /**
     * Returns an <code>ImageWriter</code>corresponding to the given <code>ImageReader</code>, if there is one, or
     * <code>null</code> if the plug-in for this <code>ImageReader</code> does not specify a corresponding
     * <code>ImageWriter</code>, or if the given <code>ImageReader</code> is not registered. This mechanism may be used
     * to obtain an <code>ImageWriter</code> that will understand the internal structure of non-pixel metadata (as
     * encoded by <code>IIOMetadata</code> objects) generated by the <code>ImageReader</code>. By obtaining this data
     * from the <code>ImageReader</code> and passing it on to the <code>ImageWriter</code> obtained with this method, a
     * client program can read an image, modify it in some way, and write it back out preserving all metadata, without
     * having to understand anything about the structure of the metadata, or even about the image format. Note that this
     * method returns the "preferred" writer, which is the first in the list returned by
     ...
     *
     * @exception IllegalArgumentException if <code>reader</code> is <code>null</code>.
     *
    ...
    /**
     * Returns an <code>ImageReader</code>corresponding to the given <code>ImageWriter</code>, if there is one, or
     * <code>null</code> if the plug-in for this <code>ImageWriter</code> does not specify a corresponding
     * <code>ImageReader</code>, or if the given <code>ImageWriter</code> is not registered. This method is provided
     * principally for symmetry with <code>getImageWriter(ImageReader)</code>. Note that this method returns the
     * "preferred" reader, which is the first in the list returned by
    ...
    /**
     * Returns an <code>Iterator</code> containing all currently registered <code>ImageWriter</code>s that claim to be
     * able to encode images of the given layout (specified using an <code>ImageTypeSpecifier</code>) in the given
     * format.
     *
     * @param type an <code>ImageTypeSpecifier</code> indicating the layout of the image to be written.
    ...
    /**
     * Returns an <code>Iterator</code> containing all currently registered <code>ImageTranscoder</code>s that claim to
     * be able to transcode between the metadata of the given <code>ImageReader</code> and <code>ImageWriter</code>.
     *
     ...
     *
     * @exception IllegalArgumentException if <code>reader</code> or <code>writer</code> is <code>null</code>.
     */
     ...
     * wrapped in an <code>ImageInputStream</code>. If no registered <code>ImageReader</code> claims to be able to read
     ...
     *
     * <p>
     * Note that there is no <code>read</code> method that takes a filename as a <code>String</code>; use this method
     * instead after creating a <code>File</code> from the filename.
     *
     * <p>
     * This method does not attempt to locate <code>ImageReader</code>s that can read directly from a <code>File</code>;
     * that may be accomplished using <code>IIORegistry</code> and <code>ImageReaderSpi</code>.
     *
    ...
    /**
     * Returns a <code>BufferedImage</code> as the result of decoding a supplied <code>InputStream</code> with an
     * <code>ImageReader</code> chosen automatically from among those currently registered. The <code>InputStream</code>
     * is wrapped in an <code>ImageInputStream</code>. If no registered <code>ImageReader</code> claims to be able to
     * read the resulting stream, <code>null</code> is returned.
     *
     * <p>
     * The current cache settings from <code>getUseCache</code>and <code>getCacheDirectory</code> will be used to
     * control caching in the <code>ImageInputStream</code> that is created.
     ...
     *
     * <p>
     * This method <em>does not</em> close the provided <code>InputStream</code> after the read operation has completed;
     ...
     * <code>ImageReader</code> chosen automatically from among those currently registered. An <code>InputStream</code>
     * is obtained from the <code>URL</code>, which is wrapped in an <code>ImageInputStream</code>. If no registered
     ...
     *
     * <p>
     * This method does not attempt to locate <code>ImageReader</code>s that can read directly from a <code>URL</code>;
     * that may be accomplished using <code>IIORegistry</code> and <code>ImageReaderSpi</code>.
     *
     ...
     * <code>ImageReader</code> claims to be able to read the stream, <code>null</code> is returned.
     *
     * <p>
     * Unlike most other methods in this class, this method <em>does</em> close the provided
     * <code>ImageInputStream</code> after the read operation has completed, unless <code>null</code> is returned, in
     * which case this method <em>does not</em> close the stream.
     *
     ...
     *
     * <p>
     * This method <em>does not</em> close the provided <code>ImageOutputStream</code> after the write operation has
     ...
     *
     * <p>
     * This method <em>does not</em> close the provided <code>OutputStream</code> after the write operation has
     * completed; it is the responsibility of the caller to close the stream, if desired.
     *
     * <p>
     * The current cache settings from <code>getUseCache</code>and <code>getCacheDirectory</code> will be used to
    ...
    /**
     * Returns <code>ImageWriter</code> instance according to given rendered image and image format or <code>null</code>
     * if there is no appropriate writer.
     */
