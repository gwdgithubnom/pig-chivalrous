...
public class IdcardCommand {
    ...
    public static String convert15To18(String idCard) {
        ...
        if (Validator.isNumber(idCard)) {
            ...
            Date birthDate = DateTimeCommand.parse(birthday, "yyMMdd");
            // 获取出生年(完全表现形式,如：2010)
            int sYear = DateTimeCommand.year(birthDate);
            idCard18 = StringCommand.builder().append(idCard.substring(0, 6)).append(sYear).append(idCard.substring(8));
            // 获取校验位
    ...
    public static boolean isValidCard(String idCard) {
        ...
        switch (length) {
            case 18:
                return isvalidCard18(idCard); // 18位身份证
            case 15:
                return isvalidCard15(idCard); // 15位身份证
            case 10: {
                // 10位身份证，港澳台地区
    ...
    public static boolean isvalidCard15(String idCard) {
        ...
        if (Validator.isNumber(idCard)) {
            ...
            // 生日
            DateTime birthDate = DateTimeCommand.parse(idCard.substring(6, 12), "yyMMdd");
    ...
    public static int getAgeByIdCard(String idCard) {
        return getAgeByIdCard(idCard, DateTimeCommand.dateTime());
    }
    ...
    public static int getAgeByIdCard(String idCard, Date dateToCompare) {
        ...
        return DateTimeCommand.age(DateTimeCommand.parse(birth, "yyyyMMdd"), dateToCompare);
    }
    ...
    private static char getCheckCode18(int iSum) {
        char sCode = StringCommand.C_SPACE;
        switch (iSum % 11) {
            case 10:
                sCode = '2';
                break;
