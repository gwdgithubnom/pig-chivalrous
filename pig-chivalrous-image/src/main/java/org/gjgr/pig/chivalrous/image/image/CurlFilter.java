...
public class CurlFilter extends TransformFilter {
    ...
    protected void transformInverse(int x, int y, float[] out) {
        /*
         * Fisheye float mirrorDistance = width*centreX; float mirrorRadius = width*centreY; float cx = width*.5f; float
         * cy = height*.5f; float dx = x-cx; float dy = y-cy; float r2 = dx*dx+dy*dy; float r = (float)Math.sqrt( r2 );
         * float phi = (float)(Math.PI*.5-Math.asin( Math.sqrt( mirrorRadius*mirrorRadius-r2 )/mirrorRadius )); r = r >
         * mirrorRadius ? width : mirrorDistance * (float)Math.tan( phi ); phi = (float)Math.atan2( dy, dx ); out[0] =
         * cx + r*(float)Math.cos( phi ); out[1] = cy + r*(float)Math.sin( phi );
         */
        float t = transition;
        float px = x;
        float py = y;
        ...
        for (int y = 0; y < outHeight; y++) {
            for (int x = 0; x < outWidth; x++) {
                ...
                if (srcX >= 0 && srcX < srcWidth1 && srcY >= 0 && srcY < srcHeight1) {
                    ...
                    nw = inPixels[i];
                    ne = inPixels[i + 1]; // LensBlurFilter
