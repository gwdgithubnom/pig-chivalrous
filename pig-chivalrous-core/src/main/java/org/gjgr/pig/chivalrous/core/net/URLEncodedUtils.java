...
public class URLEncodedUtils {
    ...
    static {
        ...
        PATHSAFE.set(';'); // paramMap separator
    ...
    public static String format(
        ...
        return format(parameters, QP_SEP_A, Consts.UTF_8, true);
    }
    ...
    public static String format(
            final List<? extends NameValuePair> parameters,
            final String charset) {
        return format(parameters, QP_SEP_A, charset != null ? Charset.forName(charset) : Consts.UTF_8, true);
    }

    public static String format(
            final List<? extends NameValuePair> parameters,
            final boolean blankAsPlus) {
        return format(parameters, QP_SEP_A, Consts.UTF_8, blankAsPlus);
    }

    public static String format(
            final List<? extends NameValuePair> parameters,
            final Charset charset, final boolean blankAsPlus) {
        return format(parameters, QP_SEP_A, charset, blankAsPlus);
    }

    public static String format(
            final List<? extends NameValuePair> parameters,
            final Charset charset, final boolean blankAsPlus, boolean hexAsPlus) {
        return format(parameters, QP_SEP_A, charset, blankAsPlus, hexAsPlus);
    }
    ...
    public static String format(
            ...
            final Charset charset, boolean blankAsPlus, boolean hexAsPlus) {
        final StringBuilder result = new StringBuilder();
        for (final NameValuePair parameter : parameters) {
            if (result.length() > 0) {
                result.append(parameterSeparator);
            }
            result.append(format(parameter, charset, blankAsPlus, hexAsPlus));
        }
        return result.toString();
    }

    public static String format(NameValuePair parameter, boolean blankAsPlus, boolean hexAsPlus) {
        return format(parameter, Consts.UTF_8, blankAsPlus, hexAsPlus);
    }

    public static String format(
            NameValuePair parameter, final Charset charset, boolean blankAsPlus, boolean hexAsPlus) {
        StringBuilder result = new StringBuilder();
        if (hexAsPlus) {
            final String encodedName = encodeFormFields(parameter.getName(), charset, blankAsPlus);
            final String encodedValue = encodeFormFields(parameter.getValue(), charset, blankAsPlus);
    ...
    public static String format(
            final List<? extends NameValuePair> parameters,
            final char parameterSeparator,
            final Charset charset, boolean blankAsPlus) {

        return format(parameters, parameterSeparator, charset, blankAsPlus, true);
    }
     ...
     * <p>
     * Uses the {@link #URLENCODER} set of characters, rather than the {@link #UNRESERVED} set; this is for compatibilty
    ...
    private static String encodeFormFields(final String content, final String charset) {
        ...
        return encodeFormFields(content, charset != null ? Charset.forName(charset) : Consts.UTF_8, true);
    }

    private static String encodeFormFields(final String content, final String charset, final boolean blankAsPlus) {
        if (content == null) {
            return null;
        }
        return encodeFormFields(content, charset != null ? Charset.forName(charset) : Consts.UTF_8, blankAsPlus);
    }

    private static String encodeFormFields(final String content, final Charset charset, final boolean blankAsPlus) {
        if (content == null) {
            return null;
        }
